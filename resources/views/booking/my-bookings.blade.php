...
<div class="p-6 lg:p-8 max-w-5xl mx-auto">
    ...
    <div class="mb-6 animate-fade-up">
        <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight">Riwayat Booking</h1>
        <p class="text-sm text-gray-500 mt-0.5">Semua riwayat dan status booking kamu.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center animate-fade-up">
        ...
        <p class="text-gray-500 font-medium mb-4">Belum ada booking.</p>
        <a href="{{ route('user.booking.index') }}"
           class="inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-semibold text-sm px-5 py-2.5 rounded-xl shadow-md transition-all">
            Booking Sekarang
        </a>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden animate-fade-up"
            ...
            <div class="flex flex-col sm:flex-row">
                ...
                {{-- Field color block --}}
                <div class="sm:w-36 h-28 sm:h-auto bg-gradient-to-br from-brand-400 to-brand-700 flex-shrink-0 flex items-center justify-center">
                    ...
                    @else
                    <svg class="w-10 h-10 text-white/50" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/><path d="M12 6l1.5 4.5H18l-3.75 2.73 1.43 4.27L12 14.54l-3.68 2.96 1.43-4.27L6 10.5h4.5z" fill="white"/></svg>
                    @endif
                </div>
                ...
                <div class="flex-1 px-5 py-4">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <p class="font-mono text-[10px] text-gray-400 mb-0.5">{{ $booking->reservation_code }}</p>
                            <h3 class="font-bold text-gray-900 text-sm">{{ $booking->field->name ?? '–' }}</h3>
                        </div>
                        <span class="flex-shrink-0 text-[11px] font-bold px-2.5 py-1 rounded-full border {{ $badgeCls }}">
                            {{ $badge['label'] }}
                        </span>
                    </div>
                ...
                {{-- Actions --}}
                <div class="px-4 py-4 sm:flex sm:items-center border-t sm:border-t-0 sm:border-l border-gray-100 flex-shrink-0">
                    <div class="flex sm:flex-col gap-2">
                        @if($booking->status === 'pending' && !$booking->payment)
                        <a href="{{ route('user.payment.show', $booking->id) }}"
                           class="inline-flex items-center gap-1.5 bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold px-3 py-2 rounded-lg transition-all whitespace-nowrap">
                            Bayar
                        </a>
                        @endif
    ...
    <div class="mt-6 animate-fade-up">{{ $bookings->links() }}</div>
    @endif
    @endif
